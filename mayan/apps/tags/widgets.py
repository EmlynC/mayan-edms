...
def get_tags_inline_widget(document):
    ...
    if tag_count:
        ...
        for tag in document.tags.all():
            tags_template.append(u'<ul class="tags list-unstyled"><li style="background: %s;" class="%s-tag">%s</li></ul>' % (tag.properties.get().get_color_code(), escape(tag.name)))
...
def get_single_tag_template(tag):
    return '<li style="background: %s" class="%s-tag">%s</li>' % (tag.properties.get().get_color_code(), escape(tag.name).replace(u' ', u'&nbsp;'))
